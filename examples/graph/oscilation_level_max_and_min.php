<?php
	...
	$data = $WDOAPI->countDaysByOscPerMaxAndMin( $days );
	...
	foreach( $data['data'] as $key => $value ){
		...
		// echo $key.': '. number_format( (100/$days) * $value , 2 ) .'% <br />';
	}
	...
	<body>
		...
		<script>
			...
			var barChartData = {
				labels: <?=$arrLabel?>,
				datasets: [{
					label: 'Numero de Vezes que foi ate essa osc max.',
			...
			window.onload = function() {
				...
				window.myBar = new Chart(ctx, {
					...
					options: {
						...
						title: {
							display: true,
							text: 'Niveis de Oscilacoes Maxima por Dias (<?=$days?> Dias)'
						}
					}
				});
